...
def analyze_video_for_facebook(record, force_regenerate: bool = False) -> dict:
    ...
    try:
        ...
        # 2. Inicializar cliente Gemini con rotación de claves
        api_key, key_record = get_gemini_key()
        client = genai.Client(api_key=api_key)

        # 3. Subir archivo a la File API de Gemini
        file_ref = client.files.upload(file=temp_video_path)

        # 4. Esperar a que el video esté listo (ACTIVE)
        ...
        # 5. Configurar prompt de estratega audiovisual para Facebook
        ...
        # Si la publicación original tenía hashtags, usamos los originales para garantizar viralidad
        if has_original_tags:
        ...
        # 7. Limpieza absoluta de recursos (servidor y Gemini)
